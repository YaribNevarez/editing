 	 ...
 	 \begin{itemize}[label={--}]
 	 	...
 	 	\begin{itemize} [label={--}]
	 		\item{Custom floating-point}.
	 	 	...
	 	 	\item{Logarithmic}.
	 	 	As shown in \fig{fig:dot_product_log}(c) in dark-gray, the pairwise product is approximated by adding $W(s|i)$ to the exponent of $h_\mu(i)$, since $W(s)$ values are represented in the logarithmic domain and $h_\mu$ in standard floating-point. In this design the pairwise product has a latency of one clock cycle.
 	 	\end{itemize}

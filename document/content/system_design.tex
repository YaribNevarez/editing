...
\REVIEW{
	In this section, we extend the system design of \mbox{\cite{nevarez2020accelerator}}, where we presented a scalable hardware architecture composed of generic homogeneous accelerator units (AUs) with standard floating-point computation. In this publication, we present an enhanced hardware architecture composed of specialized heterogeneous processing units (PUs) with hybrid custom floating-point and logarithmic dot-product approximation. The proposed approach reduces memory footprint, which allows to implement stationary synaptic weight matrix in hardware. Altogether, these innovations improve computational latency, resource utilization, and power dissipation.
	}

...
\REVIEW{
	In this section, we extend the system design of \mbox{\cite{nevarez2020accelerator}}, where we presented a scalable hardware architecture composed of generic homogeneous accelerator units (AUs) using standard floating-point arithmetic (IEEE 754) with elevated memory footprint and computational cost, which represents an overhead for error-resilient applications. Furthermore, this architecture does not implement on-chip memory for stationary synaptic weight matrix, resulting in high data movement and longer computational latency.
	
	In this publication, we present an enhanced hardware architecture composed of specialized heterogeneous processing units (PUs) using hybrid custom floating-point and logarithmic dot-product approximation with reduced computational latency. Furthermore, the proposed approach reduces the memory footprint, allowing the implementation of stationary synaptic weight matrix in hardware.
	}

Regarding the software architecture, it is structured as a
...
\REVIEW{
In order to obtain the required bit width for the number representation, we use {\equ{eq:exp_max}}, {\equ{eq:bits_exp}}, and {\equ{eq:bits_bitwidth}}. Where {\equ{eq:exp_max}} obtains the smallest entry value from the synaptic weight matrix, {\equ{eq:bits_exp}} obtains the bit width to represent the integer exponent of the smallest synaptic weight value, and {\equ{eq:bits_bitwidth}} obtains the total bit width required for the number representation of the input synaptic weight vector $\vec{W}(s_t)$, which incorporates exponent and mantissa bit widths (see {\fig{fig:product_unit_bitwidth}}). $N_M$ denotes the mantissa bit width, this is a knob parameter that is tuned by the designer to trade-off between resource utilization and QoR.
}
